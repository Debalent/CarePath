...
export default function DriverMessagesPage() {
  ...
  const send = async (e: FormEvent) => {
    ...
    if (!text.trim()) return
    if (mode === 'demo') {
      setMessages(prev => [...prev, { id: `demo-${Date.now()}`, channel: 'portal', direction: 'outbound', message: text, status: 'sent', relatedId: rideId, createdAt: new Date().toISOString(), user: { firstName: 'You', lastName: '', role: 'DRIVER' } }])
      setText(''); return
    }
    if (!token) { setError('Token required.'); return }
    setSending(true)
  ...
  return (
    <DashboardLayout role="driver" title="Messages" subtitle="Communication with coordinators and patients" userName="Driver">
    ...
    color: "#ffffff",}}>
          ...
          <h2 style={{fontSize: 20, fontWeight: 800, color: '#fff' }}>Stay in sync with coordinators and patients about your assigned rides.</h2>
        </section>
        ...
        <Card>
          <div className="cp-space-y-3">
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <Button size="sm" variant="secondary" onClick={() => { const t = localStorage.getItem('carepath.driver.token'); if (t) setToken(t) }}>Load token</Button>
              <Button size="sm" style={{ background:" #5d7db6", color: "fffff", }} onClick={() => { localStorage.setItem('carepath.driver.token', token) }}>Save token</Button>
